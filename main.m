...
%% Importing fuzzy logic files

fuzzyselect=readfis('selectroad.fis'); % import fuzzy controller for select of green light
fuzzygreentime=readfis('greentime.fis'); % import fuzzy controller for duration of green light
figure1=figure;
%% Parameters for the simulation

x=[10 35 12 40];        % initial condition (no. of cars at initial condition)
y=[0 0 0 0];            % y is a matrix that contain light state (0=Red, 1=Green, 0.5=Yellow)
it=10;                  % #loops
timefactor=ones(1,9);   % each light must be green after 10 unit steps
b=zeros(0,it);          % an auxiliary matrix for determine which light must be green

yellowtime=1;            % times (in second) for yellow light
exittime=0.5;           % times (in second) for each exit cars

entercar=2;             % #average of cars that enter to each street
exitcar=6;              % #cars that exit in each step from green light
coeff=[1 2 1 2];        % determine coefficient for entercar for each street

%% Creating figures for the simulation and the applying the setup
for ii=2:it
   
    clf('reset');  
    map(x,y,figure1);                % creatfigure1 is a function that builds 4 streets
...
if x<[60 60 60 60]      % domain of fuzzy controller (if x>[60 60 60 60] control is out of fuzzy controller and control will be by MAX function)
    ...
    for m=1:9           % each light must be green after 10 step
        if timefactor(m)>=10
        y=zeros(1,4);
        timefactor(m)=0;
        b(ii)=m;
    ...
    timefactor=timefactor+ones(1,9);
    timefactor(b(ii))=0;
...
if b(ii)~=b(ii-1) & ii~=2
    ...
    y(b(ii))=0;      % red
    trafficlight(x,y,figure1);         % creatfigure2 is a function that shows number of cars and lights
    pause(yellowtime);
    y(b(ii-1))=0;    %red
    y(b(ii))=1;      %green
end
